...
def parse_js(js: str):
    ...
    # Coleta voos de TODAS as seções disponíveis
    flight_sections = []
    for payload_idx in [2, 3]:
        if payload[payload_idx] and isinstance(payload[payload_idx], list):
            for section_idx, section in enumerate(payload[payload_idx]):
                if isinstance(section, list) and len(section) > 0:
                    # Valida se a seção contém voos (não apenas inteiros ou outros tipos)
                    if isinstance(section[0], list) and len(section[0]) >= 2:
                        flight_sections.append(section)
